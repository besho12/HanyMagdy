...
class Student_model extends MY_Model
{
    ...
    public function save($data = array(), $getBranch = array())
    {
        ...
        $inser_data1 = array(
            ...
            'parent_mobileno' => $this->input->post('parent_mobileno'),
        ...
        if (!isset($data['student_id']) && empty($data['student_id'])) {
            if (!isset($data['guardian_chk']) || 1 == 1) {
                // add new guardian all information in db
                if (!empty($data['grd_name']) || !empty($data['father_name']) || 1 == 1) {
//                    $arrayParent = array(
...
//                    );
                    ...
                    $arrayParent = array(
                        'name' => "Test " . time(),
                        'relation' => "Test " . time(),
                        'father_name' => "Test " . time(),
                        'mother_name' => "Test " . time(),
                        'occupation' => "Test " . time(),
                        'income' => "Test " . time(),
                        'education' => "Test " . time(),
            ...
            $string_student_id = '';
            if ($student_id < 10) {
                $string_student_id = '000000'.$student_id;
            } else if ($student_id < 100) {
                $string_student_id = '00000'.$student_id;
            } else if ($student_id < 1000) {
                $string_student_id = '0000'.$student_id;
            } else if ($student_id < 10000) {
                $string_student_id = '000'.$student_id;
            } else if ($student_id < 100000) {
                $string_student_id = '00'.$student_id;
            } else if ($student_id < 1000000) {
                $string_student_id = '0'.$student_id;
            } else if ($student_id < 10000000) {
                $string_student_id = $student_id;
            }

            $this->db->where('id', $student_id);
            // $this->db->update('student', array(
            //     'register_no' => 'HM2024-'.$string_student_id
            // ));
...
//            );
        ...
        $inser_data1 = array(
            ...
            'caste' => $row['Caste'],
            'mobileno' => $row['Phone'],
            'parent_mobileno' => $row['GuardianMobileNo'],
            'notes' => $row['Notes'],
            'city' => $row['City'],
            'state' => $row['State'],
