...
class Card_manage_model extends MY_Model
{
    ...
    public function tagsReplace($roleID, $userID, $templete, $print_date, $expiry_date)
    {
        ...
        // Generate barcode
        //Zend_Barcode::render('code128', 'image', array('text'=>ucfirst($qr_code) . " - " . $userDetails[$qr_code]), array());
        $qrData = str_replace('=', '', base64_encode('e-' . $userDetails['id']));
        $file = Zend_Barcode::draw('code128', 'image', array('text' => $qrData), array());
        //$code = time().$code;
        $store_image = imagepng($file,'uploads/barcode/stu_' . $userDetails['id'] . '.png');
        $body .= '<br><img src="'.base_url('uploads/barcode/stu_' . $userDetails['id'] . '.png').'">';
        return $body;
    }
